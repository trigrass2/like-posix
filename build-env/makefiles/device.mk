...
# limit the list to ones that have definitions below
...
LDSCRIPT = stm32.ld
LINKERSCRIPTPATH = $(OUTDIR)

## configure device source files
## note that STARTUP_SOURCE may only contain a single file for startup code.
# note: startup.c is #included in ../CMx/interrupt_handlers_xx.c
#STARTUP_SOURCE = $(DEVICEDIR)/startup.c
STARTUP_SOURCE += $(CHIPSUPPORTDIR)/$(INTERRUPT_HANDLER_SOURCE)
## configure other devicedir source
SOURCE += $(DEVICEDIR)/device.c
SOURCE += $(DEVICEDIR)/cortex_m_exception.c
SOURCE += $(DEVICEDIR)/systick.c
...
CFLAGS += -D $(DEVICE)
CFLAGS += -D DEBUG_PRINTF_EXCEPTIONS=$(DEBUG_PRINTF_EXCEPTIONS)
...
buildlinkerscript:
	...
	@cp $(DEVICEDIR)/$(BASE_LINKER_SCRIPT).in $(LINKERSCRIPTPATH)/$(BASE_LINKER_SCRIPT)
	@cp $(DEVICEDIR)/$(LDSCRIPT).in $(LINKERSCRIPTPATH)/$(LDSCRIPT)
	@sed -i 's/!DEVICE_STRING!/$(DEVICE)/g' $(LINKERSCRIPTPATH)/$(LDSCRIPT)
	@sed -i 's/!DATE!/$(DATE)/g' $(LINKERSCRIPTPATH)/$(LDSCRIPT)
